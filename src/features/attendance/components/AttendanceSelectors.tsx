...
export interface AttendanceSelectorsProps {
  control: Control<AttendanceFormData>;
  disabled?: boolean;
}

/**
 * Seletores de Paciente e Protocolo Mestre.
 ...
 */
export function AttendanceSelectors({ control, disabled = false }: AttendanceSelectorsProps) {
        ...
        render={({ field }) => (
          <FormItem>
            ...
            {protocolsUnavailable && (
              <UnavailableHint message="Fonte de Protocolos Mestres indisponível (Aguardando BKG v3.0). A sessão pode ser registrada sem vínculo de protocolo." />
            )}
            <FormMessage />
          </FormItem>
        )}
